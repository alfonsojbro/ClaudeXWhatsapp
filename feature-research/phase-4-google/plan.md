...
export interface PendingAction<P = unknown> {
  token: string;
  kind: string; // e.g. 'gmail_send' | 'calendar_create_event' | 'calendar_update_event' | 'whatsapp_send'
...
export async function checkGoogleToken(
  cfg: Pick<GoogleConfig, 'clientId' | 'clientSecret' | 'refreshToken' | 'tokenUrl'>,
...
- `docs/RUNBOOK.md` append `## 8. Google OAuth (Phase 4)`: create GCP project → enable Gmail API, Calendar API, People API →
  OAuth consent screen: External, **publish to Production** (unverified is fine for one personal account; in _Testing_
