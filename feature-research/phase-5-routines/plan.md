...
### Brain job runner (`runner/brain.ts`) — Agent SDK (see `sdk-notes.md`)
- `query({ prompt, options })` with: `model` (mapped id), `cwd: CXW_WORKSPACE_DIR`, `maxTurns`, `permissionMode: 'dontAsk'` (headless; unlisted tools are denied without prompting), `allowedTools: [...tools.map(t => \`mcp__${t}__*\`), 'Read', 'Grep', 'Glob']`, `disallowedTools: ['Bash','Write','Edit','MultiEdit','NotebookEdit','WebFetch','WebSearch','Task']`, `mcpServers` = entries from `workspace/.mcp.json` filtered by `tools` (so only the servers a routine needs are started), `settingSources: []` plus `systemPrompt` = Claude Code preset with `append` = contents of `workspace/CLAUDE.md` when the file exists (fallback: `settingSources: ['project']` if the preset/append form is not in the installed `.d.ts`), `abortController`. No `resume`/`sessionId`: every run is a fresh session.
  - **The implementer must verify option names and the result-message shape against the installed SDK's `.d.ts`** (`node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts`) and adjust; the names above come from the doc snapshot in `sdk-notes.md`.
- Iterate the async iterable; on `type:'result'`: `subtype:'success'` → `{ text: result, costUsd: total_cost_usd, numTurns: num_turns, sessionId: session_id }`; any other subtype → `{ isError: true, error: subtype }`. If the iterable ends without a result message → error `no_result`.
- Wall-clock limit `JOB_TIMEOUT_MS` (default 15 min) via the AbortController; lease loss also aborts.
- `main.ts` fails fast at startup when neither `ANTHROPIC_API_KEY` nor `CLAUDE_CODE_OAUTH_TOKEN` is set, unless every enabled routine is `kind: health|static`.
